    ...
    <div className="bg-background text-primary min-h-screen selection:bg-accent selection:text-black">
      ...
      <main>
        ...
        <section className="relative w-full h-[90vh] overflow-hidden flex items-end">
          ...
          <div className="relative z-10 px-[6vw] pb-24 max-w-4xl space-y-8">
            <motion.div
            ...
            >
              ...
              <h1 className="font-display text-[clamp(4rem,12vw,15rem)] leading-[0.85] text-huge uppercase tracking-tight">
                {film.title}
              </h1>
              
              {film.basadaEn && (
                <p className="text-[12px] font-sans tracking-[0.2em] uppercase text-primary/40 font-light">
            ...
            >
              ...
              >
                <div className="w-12 h-12 rounded-full border border-accent flex items-center justify-center group-hover:bg-accent transition-all duration-500">
          ...
          {/* Side Title (Industrial Decoration) */}
        ...
        {/* CONTENT GRID */}
        <section className="px-[6vw] py-32 grid grid-cols-1 lg:grid-cols-20 gap-16 items-start">
          ...
          >
            ...
            <div className="space-y-6">
              ...
              <p className="font-serif italic text-xl leading-relaxed text-primary/90 border-l-2 border-accent/30 pl-8">
                "{film.sinopsis}"
              </p>
            </div>
          </motion.div>
          ...
          >
            <div className="mb-16">
              <h2 className="font-display text-6xl uppercase tracking-tight mb-8">FICHA TÉCNICA</h2>
            ...
            {film.protagonistas && film.protagonistas.length > 0 && (
              <div className="mt-32">
                <h2 className="font-display text-6xl uppercase tracking-tight mb-8">ELENCO</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {film.protagonistas.map((p, i) => (
                    <div key={i} className="bg-surface/50 p-6 flex justify-between items-end border-l-2 border-accent">
                      <div className="space-y-1">
                        <span className="text-[10px] font-sans tracking-[0.2em] uppercase text-accent block">PROTAGONISTA</span>
                        <span className="text-2xl font-display uppercase tracking-[0.1em]">{p.nombre}</span>
                      </div>
                      <span className="text-[11px] font-sans tracking-[0.1em] uppercase opacity-40 italic">{p.rol}</span>
                    </div>
                  ))}
                </div>
                ...
                {film.reparto && film.reparto.length > 0 && (
                  <div className="mt-12 pt-12 border-t border-primary/10">
                    <span className="text-prestige text-primary/40 mb-8 block">REPARTO SECUNDARIO</span>
        ...
        {/* BACK NAVIGATION */}
        <section className="px-[6vw] py-24 border-t border-primary/10">
          <button 
            onClick={onBack}
            className="group flex items-center gap-8 w-fit"
          >
            <span className="font-display text-8xl uppercase tracking-tighter text-primary/20 group-hover:text-accent transition-colors duration-500">
              VOLVER AL CATÁLOGO
            </span>
            <div className="w-16 h-px bg-accent group-hover:w-32 transition-all duration-500" />
          </button>
        </section>
      ...
      {trailerOpen && (
        ...
        >
          ...
          >
            ...
            <button 
              onClick={() => setTrailerOpen(false)}
              className="absolute -top-12 right-0 text-prestige text-accent hover:text-primary transition-colors"
            >
              CERRAR [X]
            </button>
          </motion.div>
