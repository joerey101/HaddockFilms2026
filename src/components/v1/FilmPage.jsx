...
const films = [
  {
    ...
    trailer: 'https://www.youtube.com/embed/REEMPLAZAR_ID',
  ...
  },
  {
    ...
    produccion: 'Vanessa Ragone',
    sinopsis: 'El estreno más reciente de Haddock Films. Un relato que habita los bordes del horror y la literatura argentina contemporánea.',
  ...
  },
  {
    ...
    produccion: 'Vanessa Ragone',
    sinopsis: 'Basada en la novela de Claudia Piñeiro. Una madre con Parkinson busca justicia por la muerte de su hija en una Argentina que no da respuestas.',
...
const FilmPage = ({ filmId = 25, onBack, version, toggleVersion }) => {
  ...
  const anterior  = films[filmIndex + 1] || null;
  const siguiente = films[filmIndex - 1] || null;
  ...
  return (
    <div style={{
      ...
      >
        ...
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(26,26,26,0.65) 0%, transparent 50%)' }} />
        ...
        >
          <span style={{ ...micro, color: 'rgba(240,237,232,0.45)' }}>
            — {film.year}&nbsp;&nbsp;·&nbsp;&nbsp;{film.tipo}
      ...
      <div style={{
        ...
        {anterior ? (
          <motion.button
            ...
            onClick={() => onBack && onBack({ type: 'film', filmId: anterior.id })}
        ...
        <button
          onClick={() => onBack && onBack({ type: 'home' })}
        ...
        {siguiente ? (
          <motion.button
            whileHover={{ x: 4 }}
            transition={{ duration: 0.3 }}
            onClick={() => onBack && onBack({ type: 'film', filmId: siguiente.id })}
