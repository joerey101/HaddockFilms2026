...
## 🔄 TAREAS Y ROLLBACK (28 de Abril, 2026)

### 1. Migración y Reversión de Estado
**Estado:** **RESTUARADO** (Local)
- **Acción Inicial:** Se intentó realizar una migración de datos para separar completamente la Versión 1 de la Versión 2 y eliminar la navegación hacia la Versión 2 desde el menú principal.
- **Rollback:** A pedido, se revirtió el código de la carpeta `src/` al estado exacto previo a estos cambios. Se restauró el funcionamiento dual y el selector entre V1 y V2.
- **Limpieza:** Se eliminaron los archivos residuales de la migración (`haddock_movies.json`, reportes, etc.). El servidor de desarrollo en localhost queda habilitado y limpio para continuar el trabajo desde la base estable.

---
*Fin de la actualización de bitácora — 28/04/2026*

---

## 🎯 HITO DE CONSOLIDACIÓN (6 de Mayo, 2026)

### 1. Reorganización de Repositorio y Entorno Seguro
**Estado:** **RESUELTO** (Commit `c8ece1e`)
- **Problema corregido:** Se detectó que el desarrollo local de las últimas 4 semanas (donde la base de datos de películas creció de 75 a 723 líneas de información) se había estado realizando por fuera de la carpeta conectada a Git, dejando todo ese trabajo sin respaldo en la nube y evitando que Vercel se actualizara.
- **Acción:** Se movió la carpeta oculta `.git` a la raíz activa del proyecto, sincronizando de forma segura 4 semanas de avance local con GitHub.
- **Limpieza (Carpeta EXTRAS):** Todos los archivos basura, scripts viejos, copias de seguridad redundantes y exportaciones fallidas de XML se agruparon en la carpeta `/EXTRAS`, la cual fue añadida al `.gitignore`. Esto asegura que el código fuente en GitHub permanezca ultra limpio y profesional sin eliminar ningún archivo local.

### 2. Bloqueo Definitivo de Versión (V1)
**Estado:** **RESUELTO**
- **Acción:** Quedó validado y asegurado que el enrutamiento central de `src/App.jsx` bloquea cualquier acceso a la ruta oscura (`/v2`) y redirige forzosamente hacia la versión Editorial Clara (`/v1`). 
